...
//added
static uint16_t nvme_write_amplification(FemuCtrl *n, NvmeCmd *cmd, uint32_t buf_len)
{
    ...
    trans_len = MIN(sizeof(waf), buf_len);
    if (n->host_write != 0)	
    	waf = 100 * (n->host_write + n->gc_write) / (n->host_write); 
    else
	waf = 0;
...
static uint16_t nvme_get_log(FemuCtrl *n, NvmeCmd *cmd)
{
	...
	return nvme_write_amplification(n, cmd, len);  //added
    default:
